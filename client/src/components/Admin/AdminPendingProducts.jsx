...
const AdminPendingProducts = () => {
    ...
    const fetchFarmerAndProducts = async () => {
        ...
        try {
            // Fetch Farmer Data
            const productsResponse = await axios.get(`${baseURL}/api/pendingproducts`);
            if (productsResponse.status === 200) {
                // console.log('productsResponse.data.farmer', productsResponse.data.products);
    ...
    return (
        <div className=" px-4 py-6 sm:px-6 lg:px-8">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Pending For approval</h2>
            <div className="grid grid-cols-1 sm:gap-4 sm:grid-cols-2 md:grid-cols-3 md:gap-6 lg:grid-cols-4 lg:gap-8">
                {products.length > 0 ? (
                    ...
                    <p className="text-gray-500">No products available for approval.</p>
                )}
            </div>
        </div>
    );
