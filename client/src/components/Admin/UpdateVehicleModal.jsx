...
const UpdateVehicleModal = ({ vehicle, isOpen, onClose, onVehicleUpdated }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        bestVehicle: vehicle.bestVehicle !== undefined ? vehicle.bestVehicle : false,
    ...
    const handleSubmit = async (e) => {
        ...
        updatedVehicleData.append("available", String(formData.available));

        if (role === "admin") {
            updatedVehicleData.append("bestVehicle", String(formData.bestVehicle));
        }
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md border border-gray-200 max-h-[90vh] flex flex-col">
                <h2 className="text-lg font-semibold mb-4 text-green-700 text-center">Update Vehicle</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 flex flex-col flex-grow overflow-y-auto pr-2">
                    ...
                    <div>
                        <label className="block text-sm text-gray-700 font-medium mb-1">License Plate</label>
                        <input
                            type="text"
                            ...
                            onChange={handleChange}
                            className="w-full p-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-700"
                            placeholder="Enter license plate (e.g., LHR-1234)"
                            required
                        />
                    </div>
                    <div>
