...
const TransporterPendingVehicles = () => {
    const { id } = useAuth();
    // console.log(id);
    ...
    const fetchTransporterAndVehicles = async () => {
        ...
        try {
            ...
            if (vehiclesResponse.status === 200) {
                ...
                const unapprovedVehicles = data.filter(vehicle => vehicle.approved === false);
                setVehicles(unapprovedVehicles);
        ...
        <div className="px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 sm:gap-4 sm:grid-cols-2 md:grid-cols-3 md:gap-6 lg:grid-cols-4 lg:gap-8">
                {vehicles.length > 0 ? (
                    ...
                    <p className="text-gray-500">No vehicles available for approval.</p>
                )}
            </div>
        </div>
    );
...
export default TransporterPendingVehicles;