...
const RelatedFarmerProducts = ({ farmer }) => {
    ...
    useEffect(() => {
        const fetchRelatedProducts = async () => {
            try {
                const res = await axios.get(`${baseURL}/api/farmerproducts/${farmer._id}`);
    ...
    return (
        <div className="py-4 px-4">
            <h2 className="text-2xl font-semibold mb-6 text-green-800 text-center">{`Other Product by ${farmer.firstName} ${farmer.lastName}`}</h2>
...
export default RelatedFarmerProducts;
